...
def test_lpca_results(data, ver):
    assert skdim.id_flex.lPCA(ver=ver).fit(
        data, nbhd_type="eps", radius=0.5
    ).dimension_ == pytest.approx(5, 0.5)
...
def test_on_swiss_roll(ver):
    ...
    estim_dim = pca.fit_transform(swiss_roll_dat[0], nbhd_type="knn", n_neighbors=20)
